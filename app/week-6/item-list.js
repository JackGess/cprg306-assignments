    ...
    <section className="p-5 bg-gradient-to-br from-blue-700 to-indigo-700 text-white rounded-xl shadow-lg max-w-3xl mx-auto space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button active={sortKey === "name"} onClick={() => onSortClick("name")}>Name</Button>
          <Button active={sortKey === "category"} onClick={() => onSortClick("category")}>Category</Button>
          <Button active={sortKey === "quantity"} onClick={() => onSortClick("quantity")}>Quantity</Button>
          <DirPill />
        </div>
      </div>
